...
<%
    ...
    Corte corte = (Corte) request.getAttribute("corte");
    boolean isEdicao = (corte != null);
%>
...
<head>
    ...
    <title><%= isEdicao ? "Editar" : "Cadastrar" %> Corte - Admin</title>
    ...
    <div class="admin-wrapper">
        ...
        <div class="main-content">
            ...
            <main class="content-body">
                <div class="page-header"><h1><%= isEdicao ? "Editar Corte" : "Novo Corte" %></h1></div>
                <div class="form-container">
                    <form action="corte" method="post">
                        <input type="hidden" name="action" value="<%= isEdicao ? "editar" : "cadastrar" %>">
                        <% if (isEdicao) { %><input type="hidden" name="id" value="<%= corte.getId_corte() %>"><% } %>

                        <div class="form-group">
                            <label for="nome">Nome do Corte</label>
                            <input type="text" id="nome" name="nome" value="<%= isEdicao ? corte.getNome_corte() : "" %>" required>
                        </div>
                        <div class="form-group">
                            <label for="valor">Valor (ex: 45.00)</label>
                            <input type="number" step="0.01" id="valor" name="valor" value="<%= isEdicao ? corte.getValor_corte() : "" %>" required>
                        </div>
                        <div class="form-group">
                            <label for="duracao">Duração (em minutos)</label>
                            <input type="number" id="duracao" name="duracao" value="<%= isEdicao ? corte.getDuracao() : "" %>" required>
                        </div>

                        <div class="form-actions">
                            <a href="corte?action=listar" class="btn btn-secondary">Cancelar</a>
